...
# Function to get server details using Shodan API (requires API key)
def get_server_details(domain, shodan_api_key):
    try:
        ip = get_ip(domain)
        ...
        return f"Error retrieving server details: {e}"

# Main function to gather domain information
def gather_domain_info(domain, shodan_api_key):
    info = {}
    ...
    info['Server Details'] = get_server_details(domain, shodan_api_key)
    return info
...
if __name__ == "__main__":
    ...
    domain_info = gather_domain_info(domain, shodan_api_key)
